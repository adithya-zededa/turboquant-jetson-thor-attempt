...
# ─── Kernel 5: TQ dequant → dense fp16 (for SDPA) ────────────────────
#
# Decompresses TQ-compressed keys to extended 2D format [MSE | QJL] and
# values to fp16, all in ONE kernel with a 2D grid (BH × token_blocks).
#
# Why this + SDPA can beat the fused kernel (kernel 4):
#   - 2D grid gives full SM utilisation: (BH × blocks) programs vs BH only
#     e.g. 8×14 = 112 programs on 20 SMs vs 8 programs on 8/20 SMs
#   - Embarrassingly parallel: no online softmax data dependency between blocks
#   - SDPA/FlashAttention handles attention with optimal tiling + tensor cores
#   - TQ provides 5x memory savings for KV storage; SDPA provides fast attention
#
# Output: K_ext (BH, N, 2D) fp16, V_out (BH, N, D) fp16
# Then caller does: SDPA(Q_ext, K_ext, V_out) with scale=1/sqrt(D)

@triton.jit
def _turboquant_dequant_kv_kernel(
    # Compressed keys
    MSE_ptr, SIGNS_ptr, NORMS_ptr, RES_NORMS_ptr, CENTROIDS_ptr,
    # Compressed values (pre-unpacked uint8)
    V_DATA_ptr, V_SCALES_ptr, V_ZEROS_ptr,
    # Output
    K_EXT_ptr,       # (BH, N_C, 2*D) fp16 — extended keys
    V_OUT_ptr,       # (BH, N_C, D) fp16 — dequantized values
    # Strides — compressed keys
    stride_m_bh, stride_m_n, stride_m_d,
    stride_s_bh, stride_s_n, stride_s_d,
    stride_n_bh, stride_n_n,
    stride_rn_bh, stride_rn_n,
    # Strides — compressed values
    stride_v_bh, stride_v_n, stride_v_d,
    stride_vs_bh, stride_vs_n, stride_vs_g,
    stride_vz_bh, stride_vz_n, stride_vz_g,
    # Strides — output keys
    stride_ke_bh, stride_ke_n, stride_ke_d,
    # Strides — output values
    stride_vo_bh, stride_vo_n, stride_vo_d,
    # Dims
    N_C,
    D: tl.constexpr,
    PACKED_D_MSE: tl.constexpr,
    ...
    # ── Dequant MSE keys: unpack + centroid gather ──
    all_packed = tl.load(
    ...
    # Build (BLOCK_N, D) MSE key tile by unpacking sub-values
    # Process VALS_PER_BYTE sub-values; each produces (BLOCK_N, PACKED_D_MSE)
    # and scatters to the correct D-dimension coordinate
    for sub in range(VALS_PER_BYTE):
        idx = (all_packed >> (sub * BITS)) & BIT_MASK
        centroid_vals = tl.load(CENTROIDS_ptr + idx).to(tl.float32)
        centroid_vals = centroid_vals * key_norms[:, None]
        coord_offsets = byte_range_mse * VALS_PER_BYTE + sub
        # Store MSE part to first D columns of K_EXT
        tl.store(
            K_EXT_ptr + pid_bh * stride_ke_bh
            + n_offs[:, None] * stride_ke_n + coord_offsets[None, :] * stride_ke_d,
            centroid_vals.to(tl.float16),
            mask=n_mask[:, None] & (coord_offsets[None, :] < D),
        )
    ...
    for bit in range(8):
        ...
        sign_vals = sign_vals * (QJL_SCALE * res_norms[:, None])
        coord_offsets = byte_range_signs * 8 + bit
        # Store QJL part to columns D..2D of K_EXT
        tl.store(
            K_EXT_ptr + pid_bh * stride_ke_bh
            + n_offs[:, None] * stride_ke_n + (D + coord_offsets[None, :]) * stride_ke_d,
            sign_vals.to(tl.float16),
            mask=n_mask[:, None] & (coord_offsets[None, :] < D),
        )
    ...
    v_dequant = v_quant * v_scale + v_zero
    tl.store(
        V_OUT_ptr + pid_bh * stride_vo_bh
        + n_offs[:, None] * stride_vo_n + d_offs[None, :] * stride_vo_d,
        v_dequant.to(tl.float16),
        mask=n_mask[:, None],
    )
...
def turboquant_dequant_kv(
    ...
    """Decompress TQ keys → extended fp16 (BH,N,2D), values → fp16 (BH,N,D).

    Uses a 2D grid (BH × token_blocks) for full SM utilisation.
    Returns (k_ext, v_out) ready for SDPA.
    """
    ...
    if mse_packed.dim() > 3:
        ...
        BH = mse_packed.shape[0]

    N_C = mse_packed.shape[1]
    D = norms.shape[-1] if norms.dim() > 1 else mse_packed.shape[-1]
    # Infer D from centroids gather: packed_d_mse * vals_per_byte
    eff_bits, vals_per_byte = _get_packing_params(mse_bits)
    packed_d_mse = mse_packed.shape[2]
    packed_d_signs = qjl_signs.shape[2]
    D = packed_d_mse * vals_per_byte

    v_data = value_data_unpacked if value_data_unpacked is not None else value_quantized.data
    ...
    BLOCK_N = min(64, triton.next_power_of_2(N_C))

    # Allocate output tensors
    k_ext = torch.empty(BH, N_C, 2 * D, device=mse_packed.device, dtype=torch.float16)
    v_out = torch.empty(BH, N_C, D, device=mse_packed.device, dtype=torch.float16)

    # 2D grid: full SM utilisation (BH × token_blocks) vs fused kernel's (BH,)
    grid = (BH, triton.cdiv(N_C, BLOCK_N))

    _turboquant_dequant_kv_kernel[grid](
        ...
        k_ext.stride(0), k_ext.stride(1), k_ext.stride(2),
