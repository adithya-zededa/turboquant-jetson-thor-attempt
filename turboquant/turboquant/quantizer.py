...
class TurboQuantProd(torch.nn.Module):
    ...
    def __init__(
        ...
        # Precomputed combined transform: [Π^T | S^T] → one matmul for both
        # q_rot and q_sketch instead of two separate matmuls (saves a kernel launch)
        Pi_T = self.mse_quantizer.Pi.T.contiguous()
        S_T = self.S.T.contiguous()
        self.register_buffer("Pi_S_T", torch.cat([Pi_T, S_T], dim=-1))  # (D, 2D)
    ...
    def attention_score(
        ...
        Uses Triton kernels when available: rotates the *query* once (O(D²))
        instead of dequantizing all *keys* (O(N·D²)), giving ~10-30× speedup.
        Falls back to PyTorch (dequant + matmul) otherwise.
        ...
        """
        if query.is_cuda and query.dim() >= 3 and not getattr(self, '_force_pytorch', False):
            try:
                return self._triton_attention_score(query, quantized_key)
            except (ImportError, RuntimeError):
                pass
        return self._pytorch_attention_score(query, quantized_key)

    def _triton_attention_score(
        self,
        query: torch.Tensor,
        quantized_key: ProdQuantized,
    ) -> torch.Tensor:
        """Score via Triton — O(D²) query rotation instead of O(N·D²) key dequant."""
        ...
        scores_flat = turboquant_attention_score(
            query=query,
        ...
        # scores_flat is (BH, N); reshape to match input layout
        if is_4d:
            return scores_flat.reshape(B, H, 1, -1)
        # 3-D: (BH, 1, D) → (BH, 1, N)
        return scores_flat.unsqueeze(1)

    def _pytorch_attention_score(
        self,
        query: torch.Tensor,
        quantized_key: ProdQuantized,
    ) -> torch.Tensor:
        """Original PyTorch path: dequantize all keys then matmul."""
