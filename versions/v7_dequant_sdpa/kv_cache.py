...
class TurboQuantKVCache:
    ...
    def fused_attend(self, query: torch.Tensor, scale: float = None) -> torch.Tensor:
        ...
        #   1. Triton fused hybrid — fastest: streams compressed data, no
        #      materialisation, avoids O(N×2D) bandwidth cost (v6)
        #   2. Triton dequant+SDPA — TQ storage + FlashAttention (v7)
        #      (wins only at moderate ctx ~4k; materialisation hurts at long ctx)
        #   3. Triton fused compressed-only (v2)
        #   4. Extended SDPA / SDPA dequant / Tensor-core (fallbacks)
        if has_q and not has_b:
            primary = [self._fused_compressed_only]
        else:
            primary = [self._fused_hybrid]
        fallbacks = [self._triton_dequant_sdpa,
                     self._sdpa_extended_attend, self._sdpa_dequant_attend,
    ...
    # ── v7: Triton dequant → SDPA (TQ storage + FlashAttention compute) ─

    def _triton_dequant_sdpa(
        self, query: torch.Tensor, scale: float,
        has_q: bool, has_b: bool,
    ) -> torch.Tensor:
        """Triton dequant to fp16 → SDPA: TQ compression + FlashAttention.

        TurboQuant and SDPA are complementary:
          - TQ compresses KV cache 5x (memory savings)
          - SDPA/FlashAttention computes attention optimally (speed)

        The Triton dequant kernel uses a 2D grid (BH × token_blocks) for
        full SM utilisation — all 20 SMs active vs 8 in the fused kernel.
        No O(N·D²) matmuls: works in the extended 2D space (Theorem 2).
        """
        from turboquant.triton_kernels import turboquant_dequant_kv
        ...
        # Query: [q·Π^T | q·S^T] via combined matmul
        q_ext = torch.matmul(query.float(), kq.Pi_S_T).reshape(
            B, H, Q_len, 2 * D).to(dtype)

        # Triton dequant: compressed → extended fp16 keys + fp16 values
        # 2D grid (BH × blocks) gives full SM utilisation
        k_ext_c, v_c = turboquant_dequant_kv(
            quantized_key=self.key_quantized,
            value_quantized=self.value_quantized,
            centroids=kq.mse_quantizer.centroids,
            mse_bits=kq.mse_quantizer.bits,
        ...
        # Reshape to (B, H, N_c, ...)
        N_c = k_ext_c.shape[1]
        k_ext_c = k_ext_c.reshape(B, H, N_c, 2 * D)
        v_c = v_c.reshape(B, H, N_c, D)

        parts_k = [k_ext_c]
        parts_v = [v_c]

        if has_b:
            # Buffer: [k_buf·Π^T | 0] — exact since Π is orthogonal
            k_buf_rot = torch.matmul(
                self.key_buffer.float(), kq.mse_quantizer.Pi.T).to(dtype)
            k_buf_ext = F.pad(k_buf_rot, (0, D))  # zero-pad second half
            parts_k.append(k_buf_ext)
            parts_v.append(self.value_buffer)
