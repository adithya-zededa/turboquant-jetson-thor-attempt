...
class TurboQuantKVCache:
    ...
    def fused_attend(self, query: torch.Tensor, scale: float = None) -> torch.Tensor:
        ...
        # Try the fast Triton paths; fall back to PyTorch if unavailable
        try:
            if has_q and not has_b:
                return self._fused_compressed_only(query, scale)
            if not has_q and has_b:
                return F.scaled_dot_product_attention(
                    query, self.key_buffer, self.value_buffer
                )
            return self._fused_hybrid(query, scale)
        except (ImportError, RuntimeError):
            # Triton not available — fall back to dequant + matmul path
            scores = self.attention_scores(query, scale)
            weights = F.softmax(scores, dim=-1)
            return self.attend(weights)

    def _fused_compressed_only(self, query: torch.Tensor, scale: float) -> torch.Tensor:
    ...
    def _fused_hybrid(self, query: torch.Tensor, scale: float) -> torch.Tensor:
        ...
        # --- Compressed scores via Triton (avoids O(N·D²) key dequant) ----------
        kq = self.key_quantizer
        q_rot    = torch.matmul(q_flat, kq.mse_quantizer.Pi.T)   # (BH, D)  O(D²) once
        q_sketch = torch.matmul(q_flat, kq.S.T)                  # (BH, D)  O(D²) once
        ...
        # --- Combined softmax + weighted value aggregation ----------------------
        scores_all = torch.cat([scores_c, scores_b], dim=-1) * scale
        weights    = F.softmax(scores_all, dim=-1)                 # (BH, N_total)
        w_c, w_b   = weights[:, :N_c], weights[:, N_c:]

        # Compressed values: dequantize (cheap O(N·D), no rotation)
        v_c = dequantize_values(self.value_quantized, self.value_group_size)
        v_c = v_c.reshape(BH, N_c, D).float()

        # Buffer values: full precision
        v_b = self.value_buffer.reshape(BH, N_b, D).float()

        out = (torch.bmm(w_c.unsqueeze(1), v_c).squeeze(1)
             + torch.bmm(w_b.unsqueeze(1), v_b).squeeze(1))       # (BH, D)
