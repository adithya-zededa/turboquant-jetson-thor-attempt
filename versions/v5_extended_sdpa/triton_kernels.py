...
def _turboquant_fused_hybrid_kernel(
    ...
    #
    # Instead of iterating per-coordinate (256 serial iterations), we load
    # entire (BLOCK_N, PACKED_D) tiles and process them with vectorized ops.
    # MSE: 4 vectorized iterations (one per sub-value in each packed byte)
    # QJL: 8 vectorized iterations (one per sign bit)
    # Total: 12 iterations vs 256 serial — 21x fewer loop iterations.
    #
    ...
    for block_idx in range(num_c_blocks):
        ...
        # Load ALL packed bytes at once: (BLOCK_N, PACKED_D_MSE) tile
        all_packed = tl.load(
        ...
        mse_scores = tl.zeros([BLOCK_N], dtype=tl.float32)
        for sub in range(VALS_PER_BYTE):
            # Extract all indices for this sub-position: (BLOCK_N, PACKED_D_MSE)
            idx = (all_packed >> (sub * BITS)) & BIT_MASK
            # Gather centroids from L1-cached table: (BLOCK_N, PACKED_D_MSE)
            centroid_vals = tl.load(CENTROIDS_ptr + idx)
            # Matching query elements: (PACKED_D_MSE,) broadcast to (BLOCK_N, PACKED_D_MSE)
            coord_offsets = byte_range_mse * VALS_PER_BYTE + sub
            q_vals = tl.load(
                q_rot_base + coord_offsets * stride_q_d,
                mask=coord_offsets < D, other=0.0
            ).to(tl.float32)
            # Vectorized dot: reduce over PACKED_D_MSE → (BLOCK_N,)
        ...
        # Load ALL sign bytes at once: (BLOCK_N, PACKED_D_SIGNS) tile
        all_signs = tl.load(
